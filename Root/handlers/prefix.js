...
module.exports = (client, config) => {
    ...
    fs.readdirSync('./Root/commands/prefix/').forEach(dir => {
        ...
        for (let file of commands) {
            ...
            if (pull.config.name) {
                client.commands.message_prefix.set(pull.config.name, pull);
                client.console('Log', `[處理 - PREFIX] 加載了一個文件： ${ pull.config.name } (#${ client.commands.message_prefix.size })`.brightGreen);
            } else {
