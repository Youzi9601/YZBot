...
let jsonObject = window.json || {
    content: "你可以~~不~~做`以下這些事情`.```py\n和這個。\nprint('Hi')```\n*斜體*或_斜體_ __*下劃線斜體*__\n**大膽的**__**下劃線粗體**__\n***粗體斜體***__***下劃線粗體斜體***__\n__下劃線__ ~~刪除線~~",
    embed: {
        title: "你好~~人們~~世界:wave:",
        description: "您可以使用 [鏈接](https://discord.com) 或表情符號 :smile: 😎\n```\n還有代碼塊\n```",
        color: 0x41f097,
        ...
        fields: [
            {
                name: "字段 1，*Q-Q* **TAT**... (~~痛苦~~",
                value: "字段值"
            },
            {
                name: "字段 2",
                value: "您可以使用自定義表情符號 <:Kekwlaugh:722088222766923847>。 <:GangstaBlob:742256196295065661>",
                inline: false
            },
            {
                name: "行內字段",
...
addEventListener('DOMContentLoaded', () => {
        ...
        for (const child of Array.from(guiFragment.childNodes)) {
            if (child.classList?.[1] === 'content')
                ...
                for (const [i, embed] of (object.embeds.length ? object.embeds : [{}]).entries()) {
                    ...
                    guiEmbedName.querySelector('.text').innerHTML = `#${i + 1} 嵌入${embed.title ? `: <span>${embed.title}</span>` : ''}`;
        ...
        const addGuiEventListeners = () => {
            ...
            for (const e of document.querySelectorAll('.addField'))
                e.onclick = () => {
                    ...
                    if (indexOfGuiEmbed === -1) return error('找不到要將字段添加到的嵌入。');
                    ...
                    if (field) {
                        ...
                        switch (el.target.classList?.[0]) {
                            ...
                            case 'embedFooterTimestamp':
                                ...
                                if (isNaN(date.getTime())) return error('無效的日期');
                ...
                browse.onclick = e => {
                    ...
                    fileInput.onchange = el => {
                        if (el.target.files[0].size > 32 * 1024 * 1024)
                            return uploadError('文件太大。最大大小為 32 MB。', browse, 5000);
                        ...
                        fetch('https://api.imgbb.com/1/upload', { method: 'POST', body: formData })
                            ...
                            .then(res => {
                                browse.classList.remove('loading');
                                if (!res.success) {
                                    console.log('上傳失敗：', res.data?.error || res.error?.message || res);
                                    return uploadError(res.data?.error || res.error?.message || "請求失敗。 （檢查開發控制台）", browse);
                                }
                                ...
                                console.info(`${res.data.url} 將在 ${expiration /60 /60} 小時後刪除。要立即刪除它，請訪問 ${res.data.delete_url} 並向下滾動以找到刪除按鈕。`);
    ...
    document.querySelector('.top-btn.menu')?.addEventListener('click', e => {
        if (e.target.closest('.item.dataLink')) {
            ...
            if (!window.chrome)
                ...
                return prompt('這是帶有 base64 嵌入數據的當前 URL：', data);
    ...
    document.querySelector('.top-btn.copy').addEventListener('click', e => {
        ...
        if (!navigator.clipboard?.writeText(jsonData).then(next).catch(err => console.log('無法複製到剪貼板：' + err.message))) {
