  ...
  (client) => {
    client
      .on("messageCreate", (message) => {
        ...
        const count_data = countting_system.get(`${message.guild.id}`) || {
          channelid: "000",
        };
        ...
        // 接收目標數字
        let msg_num = message.content.replace(
          /[^\+\-\*\/\^\(\)\[\]\d]+/gm,
          " "
        ); // 把非字串過濾掉
        // console.log(msg_num)
        ...
        // 如果連續數數，且設定為true
          ...
          `${count_data.latestUserId}` == `${message.author.id}` &&
          `${count_data.noTwice}` == "true"
        ) {
          // 決定是否要重製
          // 輸入
          ...
          // 不錯誤重製
            ...
            message.reply({
              embeds: [
                {
                  description: `:x: 錯了喔！下一個是\`${num}\`，我們從\`${
                    num - 1
                  }\`繼續！`,
                },
              ],
      ...
      .on("messageUpdate", (oldMessage, newMessage) => {
        ...
        // 檢查是否發送
        var countting_system = new db.table("countting_system");
        const count_data = countting_system.get(`${newMessage.guild.id}`) || {
          channelid: "000",
        };
        ...
        const users_rections = oldMessage.reactions.resolve("⚠") || "none";
        ...
        if (oldMessage.content.match(/\d/g) && !botiswarning) {
          newMessage.reply({
            embeds: [
              {
                description: `:warning: 注意：這個訊息的數字為\`${
                  oldMessage.content.match(/\d+/g)[0]
                }\``,
              },
            ],
          });
      ...
      .on("messageDelete", (message) => {
        ...
        const users_rections = message.reactions.resolve("⚠") || "none";
        const isMe = users_rections.me || "none";
        let botiswarning = false;
        /* if (`${users_}`.includes(`${client.user.id}`)) {
                    botiswarning = true
        ...
        if (message.content.match(/\d/g) && !botiswarning)
          message.channel.send({
            embeds: [
              {
                description: `:warning: 注意：有個被刪除的訊息之數字為\`${
                  message.content.match(/\d+/g)[0]
                }\`...就看你相不相信了。`,
              },
            ],
          });
